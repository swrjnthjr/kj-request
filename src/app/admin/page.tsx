...
export default function AdminDashboard() {
  ...
  // Request open/close toggle states
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        setToggleError("Failed to fetch request status");
      }
    };
    fetchStatus();
  }, []);
  ...
  const handleToggle = async () => {
    ...
    try {
      const res = await fetch("/api/request-status", {
        ...
        body: JSON.stringify({ open: !isRequestOpen }),
      });
      if (!res.ok) throw new Error("Failed to update request status");
      const data = await res.json();
      setIsRequestOpen(data.open);
    } catch (e) {
      setToggleError("Failed to update request status");
    } finally {
      ...
      <div className="w-full max-w-6xl bg-gray-800 rounded-xl shadow-2xl p-6 sm:p-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            disabled={toggleLoading || isRequestOpen === null}
            className={`flex justify-center bg-violet-600 hover:bg-violet-700 text-white font-bold py-2 px-4 rounded-md transition-colors items-center gap-2 disabled:bg-violet-800 disabled:cursor-not-allowed ${
          ...
          >
            {toggleLoading ? (
              ...
              <span>🔴 Not Taking Requests</span>
            )}
          </button>
        </div>
        {toggleError && (
          <div className="bg-red-500/20 border border-red-500 text-red-300 px-4 py-2 rounded-lg text-center mb-2">
            {toggleError}
          </div>
        )}
